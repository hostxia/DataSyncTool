/**  版本信息模板在安装目录下，可自行修改。
* TCode_SerialRule.cs
*
* 功 能： N/A
* 类 名： TCode_SerialRule
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:47:02   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/

using System;

namespace DataSyncTool.IPSP.Model
{
	/// <summary>
	/// TCode_SerialRule:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class TCode_SerialRule
	{
		public TCode_SerialRule()
		{}
		#region Model
		private int _n_id;
		private string _s_iptype;
		private int? _n_order;
		private string _s_snippettype;
		private int? _n_snippetlength;
		private string _s_snippetvalue;
		private DateTime? _dt_createdate;
		private DateTime? _dt_editdate;
		private string _s_description;
		/// <summary>
		/// 
		/// </summary>
		public int n_ID
		{
			set{ _n_id=value;}
			get{return _n_id;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_IPType
		{
			set{ _s_iptype=value;}
			get{return _s_iptype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_Order
		{
			set{ _n_order=value;}
			get{return _n_order;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_SnippetType
		{
			set{ _s_snippettype=value;}
			get{return _s_snippettype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_SnippetLength
		{
			set{ _n_snippetlength=value;}
			get{return _n_snippetlength;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_SnippetValue
		{
			set{ _s_snippetvalue=value;}
			get{return _s_snippetvalue;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? dt_CreateDate
		{
			set{ _dt_createdate=value;}
			get{return _dt_createdate;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? dt_EditDate
		{
			set{ _dt_editdate=value;}
			get{return _dt_editdate;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_Description
		{
			set{ _s_description=value;}
			get{return _s_description;}
		}
		#endregion Model

	}
}

