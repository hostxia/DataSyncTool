/**  版本信息模板在安装目录下，可自行修改。
* T_LawItemPara.cs
*
* 功 能： N/A
* 类 名： T_LawItemPara
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:44:52   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/

using System;

namespace DataSyncTool.IPSP.Model
{
	/// <summary>
	/// T_LawItemPara:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class T_LawItemPara
	{
		public T_LawItemPara()
		{}
		#region Model
		private int _n_id;
		private string _s_paralocationcode;
		private string _s_paratype;
		private string _s_paravalue;
		private int _n_lawitemid;
		private byte[] _byte_paravalue;
		/// <summary>
		/// 
		/// </summary>
		public int n_ID
		{
			set{ _n_id=value;}
			get{return _n_id;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_ParaLocationCode
		{
			set{ _s_paralocationcode=value;}
			get{return _s_paralocationcode;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_ParaType
		{
			set{ _s_paratype=value;}
			get{return _s_paratype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_ParaValue
		{
			set{ _s_paravalue=value;}
			get{return _s_paravalue;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int n_LawItemID
		{
			set{ _n_lawitemid=value;}
			get{return _n_lawitemid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public byte[] byte_ParaValue
		{
			set{ _byte_paravalue=value;}
			get{return _byte_paravalue;}
		}
		#endregion Model

	}
}

