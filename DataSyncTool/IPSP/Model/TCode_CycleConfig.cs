/**  版本信息模板在安装目录下，可自行修改。
* TCode_CycleConfig.cs
*
* 功 能： N/A
* 类 名： TCode_CycleConfig
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:46:10   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/

using System;

namespace DataSyncTool.IPSP.Model
{
	/// <summary>
	/// TCode_CycleConfig:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class TCode_CycleConfig
	{
		public TCode_CycleConfig()
		{}
		#region Model
		private int _n_id;
		private int? _n_paymentcycle;
		private int? _n_lettertemplateid;
		private int? _n_emailtemplateid;
		private int _n_clientid;
		private int? _n_sequence;
		private string _s_globalconfig;
		/// <summary>
		/// 
		/// </summary>
		public int n_ID
		{
			set{ _n_id=value;}
			get{return _n_id;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_PaymentCycle
		{
			set{ _n_paymentcycle=value;}
			get{return _n_paymentcycle;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_LetterTemplateID
		{
			set{ _n_lettertemplateid=value;}
			get{return _n_lettertemplateid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_EmailTemplateID
		{
			set{ _n_emailtemplateid=value;}
			get{return _n_emailtemplateid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int n_ClientID
		{
			set{ _n_clientid=value;}
			get{return _n_clientid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_Sequence
		{
			set{ _n_sequence=value;}
			get{return _n_sequence;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_GlobalConfig
		{
			set{ _s_globalconfig=value;}
			get{return _s_globalconfig;}
		}
		#endregion Model

	}
}

