/**  版本信息模板在安装目录下，可自行修改。
* TCode_CameFile.cs
*
* 功 能： N/A
* 类 名： TCode_CameFile
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:45:52   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/

using System;

namespace DataSyncTool.IPSP.Model
{
	/// <summary>
	/// TCode_CameFile:实体类(属性说明自动提取数据库字段的描述信息)
	/// </summary>
	[Serializable]
	public partial class TCode_CameFile
	{
		public TCode_CameFile()
		{}
		#region Model
		private int _n_id;
		private string _s_iptype;
		private string _s_filetype;
		private string _s_filecode;
		private string _s_filefrom;
		private string _s_name;
		private string _s_othername;
		private int? _n_ctemplateid;
		private string _s_tapplytype;
		private int? _n_regcountry;
		private int? _n_processid;
		private string _s_notes;
		private DateTime? _dt_createdate;
		private DateTime? _dt_editdate;
		private int? _n_emailtemplateid;
		private string _s_issystem;
		private string _s_officialname;
		private string _s_officialcode;
		/// <summary>
		/// 
		/// </summary>
		public int n_ID
		{
			set{ _n_id=value;}
			get{return _n_id;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_IPType
		{
			set{ _s_iptype=value;}
			get{return _s_iptype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_FileType
		{
			set{ _s_filetype=value;}
			get{return _s_filetype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_FileCode
		{
			set{ _s_filecode=value;}
			get{return _s_filecode;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_FileFrom
		{
			set{ _s_filefrom=value;}
			get{return _s_filefrom;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_Name
		{
			set{ _s_name=value;}
			get{return _s_name;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_OtherName
		{
			set{ _s_othername=value;}
			get{return _s_othername;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_CTemplateID
		{
			set{ _n_ctemplateid=value;}
			get{return _n_ctemplateid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_TApplyType
		{
			set{ _s_tapplytype=value;}
			get{return _s_tapplytype;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_RegCountry
		{
			set{ _n_regcountry=value;}
			get{return _n_regcountry;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_ProcessID
		{
			set{ _n_processid=value;}
			get{return _n_processid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_Notes
		{
			set{ _s_notes=value;}
			get{return _s_notes;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? dt_CreateDate
		{
			set{ _dt_createdate=value;}
			get{return _dt_createdate;}
		}
		/// <summary>
		/// 
		/// </summary>
		public DateTime? dt_EditDate
		{
			set{ _dt_editdate=value;}
			get{return _dt_editdate;}
		}
		/// <summary>
		/// 
		/// </summary>
		public int? n_EmailTemplateID
		{
			set{ _n_emailtemplateid=value;}
			get{return _n_emailtemplateid;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_IsSystem
		{
			set{ _s_issystem=value;}
			get{return _s_issystem;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_OfficialName
		{
			set{ _s_officialname=value;}
			get{return _s_officialname;}
		}
		/// <summary>
		/// 
		/// </summary>
		public string s_OfficialCode
		{
			set{ _s_officialcode=value;}
			get{return _s_officialcode;}
		}
		#endregion Model

	}
}

