/**  版本信息模板在安装目录下，可自行修改。
* TCode_LetterTemplate.cs
*
* 功 能： N/A
* 类 名： TCode_LetterTemplate
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:46:34   N/A    初版
*
...
{
	/// <summary>
	/// 数据访问类:TCode_LetterTemplate
	/// </summary>
	public partial class TCode_LetterTemplate
	{
		public TCode_LetterTemplate()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("n_ID", "TCode_LetterTemplate"); 
		}
		...
		public bool Exists(int n_ID)
		{
			...
			strSql.Append("select count(1) from TCode_LetterTemplate");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(IPSP.Model.TCode_LetterTemplate model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into TCode_LetterTemplate(");
			strSql.Append("s_Name,s_FilePathName,n_LanguageID,s_OtherName,s_OfficialCode,s_Notes,dt_CreateDate,dt_EditDate)");
			...
			parameters[1].Value = model.s_FilePathName;
			parameters[2].Value = model.n_LanguageID;
			parameters[3].Value = model.s_OtherName;
			parameters[4].Value = model.s_OfficialCode;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(IPSP.Model.TCode_LetterTemplate model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update TCode_LetterTemplate set ");
		...
		public bool Delete(int n_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from TCode_LetterTemplate ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_LetterTemplate GetModel(int n_ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 n_ID,s_Name,s_FilePathName,n_LanguageID,s_OtherName,s_OfficialCode,s_Notes,dt_CreateDate,dt_EditDate from TCode_LetterTemplate ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
			...
			IPSP.Model.TCode_LetterTemplate model=new IPSP.Model.TCode_LetterTemplate();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_LetterTemplate DataRowToModel(DataRow row)
		{
			IPSP.Model.TCode_LetterTemplate model=new IPSP.Model.TCode_LetterTemplate();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select n_ID,s_Name,s_FilePathName,n_LanguageID,s_OtherName,s_OfficialCode,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_LetterTemplate ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" n_ID,s_Name,s_FilePathName,n_LanguageID,s_OtherName,s_OfficialCode,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_LetterTemplate ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from TCode_LetterTemplate T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "TCode_LetterTemplate";
