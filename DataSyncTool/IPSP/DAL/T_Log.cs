...
namespace DataSyncTool.IPSP.DAL
{
	...
	public partial class T_Log
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("n_ID", "T_Log"); 
		}
		...
		public int Add(IPSP.Model.T_Log model)
		{
			...
			strSql.Append("s_Module,s_Operator,dt_Date,s_Content,s_Type)");
			strSql.Append(" values (");
			strSql.Append("@s_Module,@s_Operator,@dt_Date,@s_Content,@s_Type)");
			strSql.Append(";select @@IDENTITY");
			SqlParameter[] parameters = {
					new SqlParameter("@s_Module", SqlDbType.NVarChar,100),
					new SqlParameter("@s_Operator", SqlDbType.NVarChar,100),
			...
			parameters[1].Value = model.s_Operator;
		...
		public bool Update(IPSP.Model.T_Log model)
		{
			...
			strSql.Append("s_Operator=@s_Operator,");
		...
		public IPSP.Model.T_Log GetModel(int n_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 n_ID,s_Module,s_Operator,dt_Date,s_Content,s_Type from T_Log ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
			...
			IPSP.Model.T_Log model=new IPSP.Model.T_Log();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.T_Log DataRowToModel(DataRow row)
		{
			IPSP.Model.T_Log model=new IPSP.Model.T_Log();
			if (row != null)
			{
				...
				if(row["s_Operator"]!=null)
				{
					model.s_Operator=row["s_Operator"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select n_ID,s_Module,s_Operator,dt_Date,s_Content,s_Type ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" n_ID,s_Module,s_Operator,dt_Date,s_Content,s_Type ");
			strSql.Append(" FROM T_Log ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
