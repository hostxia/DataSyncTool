...
namespace DataSyncTool.IPSP.DAL
{
	...
	public partial class TF_DemandTypeInTask
	{
		...
		public bool Exists(Guid g_ID)
		{
			...
			strSql.Append("select count(1) from TF_DemandTypeInTask");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into TF_DemandTypeInTask(");
			strSql.Append("g_ID,g_TaskID,n_CodeDemandTypeID)");
			...
			parameters[2].Value = model.n_CodeDemandTypeID;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update TF_DemandTypeInTask set ");
		...
		public bool Delete(Guid g_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from TF_DemandTypeInTask ");
			strSql.Append(" where g_ID=@g_ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@g_ID", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = g_ID;
		...
		public IPSP.Model.TF_DemandTypeInTask GetModel(Guid g_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 g_ID,g_TaskID,n_CodeDemandTypeID from TF_DemandTypeInTask ");
			strSql.Append(" where g_ID=@g_ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@g_ID", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = g_ID;

			IPSP.Model.TF_DemandTypeInTask model=new IPSP.Model.TF_DemandTypeInTask();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TF_DemandTypeInTask DataRowToModel(DataRow row)
		{
			IPSP.Model.TF_DemandTypeInTask model=new IPSP.Model.TF_DemandTypeInTask();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select g_ID,g_TaskID,n_CodeDemandTypeID ");
			strSql.Append(" FROM TF_DemandTypeInTask ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from TF_DemandTypeInTask T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "TF_DemandTypeInTask";
