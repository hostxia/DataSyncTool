...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:46:47   N/A    初版
*
...
namespace DataSyncTool.IPSP.DAL
{
	...
	/// 数据访问类:TCode_Process
	/// </summary>
	public partial class TCode_Process
	{
		public TCode_Process()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("n_ID", "TCode_Process"); 
		}
		...
		public bool Exists(int n_ID)
		{
			...
			strSql.Append("select count(1) from TCode_Process");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(IPSP.Model.TCode_Process model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into TCode_Process(");
			strSql.Append("s_IPType,s_Name,s_OtherName,s_AllowRepeat,s_Business,s_Notes,dt_CreateDate,dt_EditDate)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@s_AllowRepeat", SqlDbType.NVarChar,1),
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(IPSP.Model.TCode_Process model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update TCode_Process set ");
		...
		public bool Delete(int n_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from TCode_Process ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_Process GetModel(int n_ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 n_ID,s_IPType,s_Name,s_OtherName,s_AllowRepeat,s_Business,s_Notes,dt_CreateDate,dt_EditDate from TCode_Process ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
			...
			IPSP.Model.TCode_Process model=new IPSP.Model.TCode_Process();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_Process DataRowToModel(DataRow row)
		{
			IPSP.Model.TCode_Process model=new IPSP.Model.TCode_Process();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select n_ID,s_IPType,s_Name,s_OtherName,s_AllowRepeat,s_Business,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_Process ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" n_ID,s_IPType,s_Name,s_OtherName,s_AllowRepeat,s_Business,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_Process ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from TCode_Process T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "TCode_Process";
