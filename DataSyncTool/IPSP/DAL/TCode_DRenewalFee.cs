/**  版本信息模板在安装目录下，可自行修改。
* TCode_DRenewalFee.cs
*
* 功 能： N/A
* 类 名： TCode_DRenewalFee
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2017/2/28 17:46:14   N/A    初版
*
...
{
	/// <summary>
	/// 数据访问类:TCode_DRenewalFee
	/// </summary>
	public partial class TCode_DRenewalFee
	{
		public TCode_DRenewalFee()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("n_ID", "TCode_DRenewalFee"); 
		}
		...
		public bool Exists(int n_ID)
		{
			...
			strSql.Append("select count(1) from TCode_DRenewalFee");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(IPSP.Model.TCode_DRenewalFee model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into TCode_DRenewalFee(");
			strSql.Append("n_RegCountry,n_OfficialFee,n_OfficialCurrency,s_UnitPrice,s_Unit,n_ExceedUnit,n_ExceedUnitFee,s_InitialDateType,n_RenewalYear,s_Notes,dt_CreateDate,dt_EditDate)");
			...
			parameters[1].Value = model.n_OfficialFee;
			parameters[2].Value = model.n_OfficialCurrency;
			parameters[3].Value = model.s_UnitPrice;
			parameters[4].Value = model.s_Unit;
			parameters[5].Value = model.n_ExceedUnit;
			parameters[6].Value = model.n_ExceedUnitFee;
			parameters[7].Value = model.s_InitialDateType;
			parameters[8].Value = model.n_RenewalYear;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(IPSP.Model.TCode_DRenewalFee model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update TCode_DRenewalFee set ");
		...
		public bool Delete(int n_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from TCode_DRenewalFee ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_DRenewalFee GetModel(int n_ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 n_ID,n_RegCountry,n_OfficialFee,n_OfficialCurrency,s_UnitPrice,s_Unit,n_ExceedUnit,n_ExceedUnitFee,s_InitialDateType,n_RenewalYear,s_Notes,dt_CreateDate,dt_EditDate from TCode_DRenewalFee ");
			strSql.Append(" where n_ID=@n_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@n_ID", SqlDbType.Int,4)
			};
			...
			IPSP.Model.TCode_DRenewalFee model=new IPSP.Model.TCode_DRenewalFee();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.TCode_DRenewalFee DataRowToModel(DataRow row)
		{
			IPSP.Model.TCode_DRenewalFee model=new IPSP.Model.TCode_DRenewalFee();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select n_ID,n_RegCountry,n_OfficialFee,n_OfficialCurrency,s_UnitPrice,s_Unit,n_ExceedUnit,n_ExceedUnitFee,s_InitialDateType,n_RenewalYear,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_DRenewalFee ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" n_ID,n_RegCountry,n_OfficialFee,n_OfficialCurrency,s_UnitPrice,s_Unit,n_ExceedUnit,n_ExceedUnitFee,s_InitialDateType,n_RenewalYear,s_Notes,dt_CreateDate,dt_EditDate ");
			strSql.Append(" FROM TCode_DRenewalFee ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from TCode_DRenewalFee T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "TCode_DRenewalFee";
