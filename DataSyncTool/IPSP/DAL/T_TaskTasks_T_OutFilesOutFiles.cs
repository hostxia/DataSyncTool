...
{
	/// <summary>
	/// 数据访问类:T_TaskTasks_T_OutFilesOutFiles
	/// </summary>
	public partial class T_TaskTasks_T_OutFilesOutFiles
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("OutFiles", "T_TaskTasks_T_OutFilesOutFiles"); 
		}
		...
		public bool Exists(int OutFiles,int Tasks,int OID)
		{
			...
			strSql.Append("select count(1) from T_TaskTasks_T_OutFilesOutFiles");
			strSql.Append(" where OutFiles=@OutFiles and Tasks=@Tasks and OID=@OID ");
			SqlParameter[] parameters = {
					new SqlParameter("@OutFiles", SqlDbType.Int,4),
					new SqlParameter("@Tasks", SqlDbType.Int,4),
					new SqlParameter("@OID", SqlDbType.Int,4)			};
			parameters[0].Value = OutFiles;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into T_TaskTasks_T_OutFilesOutFiles(");
			strSql.Append("OutFiles,Tasks,OptimisticLockField)");
			...
			parameters[0].Value = model.OutFiles;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update T_TaskTasks_T_OutFilesOutFiles set ");
			...
			parameters[1].Value = model.OutFiles;
		...
		public bool Delete(int OID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from T_TaskTasks_T_OutFilesOutFiles ");
			strSql.Append(" where OID=@OID");
			SqlParameter[] parameters = {
					new SqlParameter("@OID", SqlDbType.Int,4)
			};
		...
		public IPSP.Model.T_TaskTasks_T_OutFilesOutFiles GetModel(int OID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 OutFiles,Tasks,OID,OptimisticLockField from T_TaskTasks_T_OutFilesOutFiles ");
			strSql.Append(" where OID=@OID");
			SqlParameter[] parameters = {
					new SqlParameter("@OID", SqlDbType.Int,4)
			};
			...
			IPSP.Model.T_TaskTasks_T_OutFilesOutFiles model=new IPSP.Model.T_TaskTasks_T_OutFilesOutFiles();
		...
		/// 得到一个对象实体
		/// </summary>
		public IPSP.Model.T_TaskTasks_T_OutFilesOutFiles DataRowToModel(DataRow row)
		{
			IPSP.Model.T_TaskTasks_T_OutFilesOutFiles model=new IPSP.Model.T_TaskTasks_T_OutFilesOutFiles();
			if (row != null)
			{
				if(row["OutFiles"]!=null && row["OutFiles"].ToString()!="")
				{
					model.OutFiles=int.Parse(row["OutFiles"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select OutFiles,Tasks,OID,OptimisticLockField ");
			strSql.Append(" FROM T_TaskTasks_T_OutFilesOutFiles ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" OutFiles,Tasks,OID,OptimisticLockField ");
			strSql.Append(" FROM T_TaskTasks_T_OutFilesOutFiles ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from T_TaskTasks_T_OutFilesOutFiles T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "T_TaskTasks_T_OutFilesOutFiles";
