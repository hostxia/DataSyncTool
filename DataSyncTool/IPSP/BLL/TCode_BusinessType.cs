...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_BusinessType dal=new IPSP.DAL.TCode_BusinessType();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_BusinessType GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_BusinessTypeModel-" + n_ID;
			...
			return (IPSP.Model.TCode_BusinessType)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_BusinessType> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_BusinessType> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_BusinessType> modelList = new List<IPSP.Model.TCode_BusinessType>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_BusinessType model;
