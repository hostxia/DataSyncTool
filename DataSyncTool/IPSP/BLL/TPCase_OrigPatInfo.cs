...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TPCase_OrigPatInfo dal=new IPSP.DAL.TPCase_OrigPatInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TPCase_OrigPatInfo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TPCase_OrigPatInfoModel-" + n_ID;
			...
			return (IPSP.Model.TPCase_OrigPatInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_OrigPatInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_OrigPatInfo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TPCase_OrigPatInfo> modelList = new List<IPSP.Model.TPCase_OrigPatInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TPCase_OrigPatInfo model;
