...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_SysTaskInFile dal=new IPSP.DAL.TCode_SysTaskInFile();
		...
		{
			return dal.Exists(n_SysTaskCodeID,n_OInfileID,n_ID);
		}
		...
		public bool Delete(int n_SysTaskCodeID,int n_OInfileID,int n_ID)
		{
			...
			return dal.Delete(n_SysTaskCodeID,n_OInfileID,n_ID);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_SysTaskInFile GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_SysTaskInFileModel-" + n_ID;
			...
			return (IPSP.Model.TCode_SysTaskInFile)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SysTaskInFile> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SysTaskInFile> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_SysTaskInFile> modelList = new List<IPSP.Model.TCode_SysTaskInFile>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_SysTaskInFile model;
