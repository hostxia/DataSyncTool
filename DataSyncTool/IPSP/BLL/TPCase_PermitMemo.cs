...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TPCase_PermitMemo dal=new IPSP.DAL.TPCase_PermitMemo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TPCase_PermitMemo GetModelByCache(int n_CaseID)
		{
			
			string CacheKey = "TPCase_PermitMemoModel-" + n_CaseID;
			...
			return (IPSP.Model.TPCase_PermitMemo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_PermitMemo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_PermitMemo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TPCase_PermitMemo> modelList = new List<IPSP.Model.TPCase_PermitMemo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TPCase_PermitMemo model;
