...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_AgencyInfo dal=new IPSP.DAL.TCode_AgencyInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_AgencyInfo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_AgencyInfoModel-" + n_ID;
			...
			return (IPSP.Model.TCode_AgencyInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_AgencyInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_AgencyInfo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_AgencyInfo> modelList = new List<IPSP.Model.TCode_AgencyInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_AgencyInfo model;
