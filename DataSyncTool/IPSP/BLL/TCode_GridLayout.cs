...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_GridLayout dal=new IPSP.DAL.TCode_GridLayout();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_GridLayout GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_GridLayoutModel-" + n_ID;
			...
			return (IPSP.Model.TCode_GridLayout)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_GridLayout> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_GridLayout> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_GridLayout> modelList = new List<IPSP.Model.TCode_GridLayout>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_GridLayout model;
