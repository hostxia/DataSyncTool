...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TTCase_SearchInfo dal=new IPSP.DAL.TTCase_SearchInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TTCase_SearchInfo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TTCase_SearchInfoModel-" + n_ID;
			...
			return (IPSP.Model.TTCase_SearchInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TTCase_SearchInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TTCase_SearchInfo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TTCase_SearchInfo> modelList = new List<IPSP.Model.TTCase_SearchInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TTCase_SearchInfo model;
