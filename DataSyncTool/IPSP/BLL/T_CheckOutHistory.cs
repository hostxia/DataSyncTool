...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.T_CheckOutHistory dal=new IPSP.DAL.T_CheckOutHistory();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.T_CheckOutHistory GetModelByCache(decimal n_ID)
		{
			
			string CacheKey = "T_CheckOutHistoryModel-" + n_ID;
			...
			return (IPSP.Model.T_CheckOutHistory)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_CheckOutHistory> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_CheckOutHistory> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.T_CheckOutHistory> modelList = new List<IPSP.Model.T_CheckOutHistory>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.T_CheckOutHistory model;
