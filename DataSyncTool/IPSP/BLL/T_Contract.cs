...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.T_Contract dal=new IPSP.DAL.T_Contract();
		public T_Contract()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.T_Contract GetModelByCache(int n_ID)
		{
			
			string CacheKey = "T_ContractModel-" + n_ID;
			...
			return (IPSP.Model.T_Contract)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_Contract> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_Contract> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.T_Contract> modelList = new List<IPSP.Model.T_Contract>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.T_Contract model;
