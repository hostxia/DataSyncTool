...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Currency dal=new IPSP.DAL.TCode_Currency();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Currency GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CurrencyModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Currency)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Currency> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Currency> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Currency> modelList = new List<IPSP.Model.TCode_Currency>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Currency model;
