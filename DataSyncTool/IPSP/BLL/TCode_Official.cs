...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Official dal=new IPSP.DAL.TCode_Official();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Official GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_OfficialModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Official)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Official> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Official> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Official> modelList = new List<IPSP.Model.TCode_Official>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Official model;
