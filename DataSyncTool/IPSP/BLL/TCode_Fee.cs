...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Fee dal=new IPSP.DAL.TCode_Fee();
		public TCode_Fee()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Fee GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_FeeModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Fee)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Fee> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Fee> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Fee> modelList = new List<IPSP.Model.TCode_Fee>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Fee model;
