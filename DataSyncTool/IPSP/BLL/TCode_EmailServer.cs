...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_EmailServer dal=new IPSP.DAL.TCode_EmailServer();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_EmailServer GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_EmailServerModel-" + n_ID;
			...
			return (IPSP.Model.TCode_EmailServer)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_EmailServer> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_EmailServer> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_EmailServer> modelList = new List<IPSP.Model.TCode_EmailServer>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_EmailServer model;
