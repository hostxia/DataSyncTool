...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TTCase_Renewal dal=new IPSP.DAL.TTCase_Renewal();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TTCase_Renewal GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TTCase_RenewalModel-" + n_ID;
			...
			return (IPSP.Model.TTCase_Renewal)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TTCase_Renewal> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TTCase_Renewal> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TTCase_Renewal> modelList = new List<IPSP.Model.TTCase_Renewal>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TTCase_Renewal model;
