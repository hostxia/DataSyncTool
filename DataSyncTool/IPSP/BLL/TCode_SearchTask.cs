...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_SearchTask dal=new IPSP.DAL.TCode_SearchTask();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_SearchTask GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_SearchTaskModel-" + n_ID;
			...
			return (IPSP.Model.TCode_SearchTask)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SearchTask> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SearchTask> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_SearchTask> modelList = new List<IPSP.Model.TCode_SearchTask>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_SearchTask model;
