...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TF_ActionInTask dal=new IPSP.DAL.TF_ActionInTask();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TF_ActionInTask GetModelByCache(Guid g_ID)
		{
			
			string CacheKey = "TF_ActionInTaskModel-" + g_ID;
			...
			return (IPSP.Model.TF_ActionInTask)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_ActionInTask> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_ActionInTask> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TF_ActionInTask> modelList = new List<IPSP.Model.TF_ActionInTask>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TF_ActionInTask model;
