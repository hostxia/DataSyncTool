...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TOCase_Other dal=new IPSP.DAL.TOCase_Other();
		public TOCase_Other()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TOCase_Other GetModelByCache(int n_CaseID)
		{
			
			string CacheKey = "TOCase_OtherModel-" + n_CaseID;
			...
			return (IPSP.Model.TOCase_Other)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TOCase_Other> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TOCase_Other> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TOCase_Other> modelList = new List<IPSP.Model.TOCase_Other>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TOCase_Other model;
