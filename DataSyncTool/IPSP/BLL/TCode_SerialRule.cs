...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_SerialRule dal=new IPSP.DAL.TCode_SerialRule();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_SerialRule GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_SerialRuleModel-" + n_ID;
			...
			return (IPSP.Model.TCode_SerialRule)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SerialRule> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SerialRule> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_SerialRule> modelList = new List<IPSP.Model.TCode_SerialRule>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_SerialRule model;
