...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCCase_CustomMemo dal=new IPSP.DAL.TCCase_CustomMemo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCCase_CustomMemo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCCase_CustomMemoModel-" + n_ID;
			...
			return (IPSP.Model.TCCase_CustomMemo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCCase_CustomMemo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCCase_CustomMemo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCCase_CustomMemo> modelList = new List<IPSP.Model.TCCase_CustomMemo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCCase_CustomMemo model;
