...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Country dal=new IPSP.DAL.TCode_Country();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Country GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CountryModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Country)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Country> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Country> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Country> modelList = new List<IPSP.Model.TCode_Country>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Country model;
