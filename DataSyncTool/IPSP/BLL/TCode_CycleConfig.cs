...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_CycleConfig dal=new IPSP.DAL.TCode_CycleConfig();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_CycleConfig GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CycleConfigModel-" + n_ID;
			...
			return (IPSP.Model.TCode_CycleConfig)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CycleConfig> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CycleConfig> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_CycleConfig> modelList = new List<IPSP.Model.TCode_CycleConfig>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_CycleConfig model;
