...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TF_Node dal=new IPSP.DAL.TF_Node();
		public TF_Node()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TF_Node GetModelByCache(Guid g_ID)
		{
			
			string CacheKey = "TF_NodeModel-" + g_ID;
			...
			return (IPSP.Model.TF_Node)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_Node> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_Node> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TF_Node> modelList = new List<IPSP.Model.TF_Node>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TF_Node model;
