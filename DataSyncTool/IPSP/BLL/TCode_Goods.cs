...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Goods dal=new IPSP.DAL.TCode_Goods();
		public TCode_Goods()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Goods GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_GoodsModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Goods)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Goods> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Goods> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Goods> modelList = new List<IPSP.Model.TCode_Goods>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Goods model;
