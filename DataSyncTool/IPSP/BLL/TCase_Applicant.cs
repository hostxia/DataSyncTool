...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCase_Applicant dal=new IPSP.DAL.TCase_Applicant();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCase_Applicant GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCase_ApplicantModel-" + n_ID;
			...
			return (IPSP.Model.TCase_Applicant)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCase_Applicant> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCase_Applicant> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCase_Applicant> modelList = new List<IPSP.Model.TCase_Applicant>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCase_Applicant model;
