...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TF_OperationLog dal=new IPSP.DAL.TF_OperationLog();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TF_OperationLog GetModelByCache(Guid g_ID)
		{
			
			string CacheKey = "TF_OperationLogModel-" + g_ID;
			...
			return (IPSP.Model.TF_OperationLog)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_OperationLog> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TF_OperationLog> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TF_OperationLog> modelList = new List<IPSP.Model.TF_OperationLog>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TF_OperationLog model;
