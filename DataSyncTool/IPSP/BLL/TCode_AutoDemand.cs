...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_AutoDemand dal=new IPSP.DAL.TCode_AutoDemand();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_AutoDemand GetModelByCache(Guid g_ID)
		{
			
			string CacheKey = "TCode_AutoDemandModel-" + g_ID;
			...
			return (IPSP.Model.TCode_AutoDemand)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_AutoDemand> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_AutoDemand> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_AutoDemand> modelList = new List<IPSP.Model.TCode_AutoDemand>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_AutoDemand model;
