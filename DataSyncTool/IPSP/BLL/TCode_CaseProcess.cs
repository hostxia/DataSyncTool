...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_CaseProcess dal=new IPSP.DAL.TCode_CaseProcess();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_CaseProcess GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CaseProcessModel-" + n_ID;
			...
			return (IPSP.Model.TCode_CaseProcess)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CaseProcess> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CaseProcess> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_CaseProcess> modelList = new List<IPSP.Model.TCode_CaseProcess>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_CaseProcess model;
