...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.T_ContactMemo dal=new IPSP.DAL.T_ContactMemo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.T_ContactMemo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "T_ContactMemoModel-" + n_ID;
			...
			return (IPSP.Model.T_ContactMemo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_ContactMemo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_ContactMemo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.T_ContactMemo> modelList = new List<IPSP.Model.T_ContactMemo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.T_ContactMemo model;
