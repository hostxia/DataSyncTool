...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_TRenewalFee dal=new IPSP.DAL.TCode_TRenewalFee();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_TRenewalFee GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_TRenewalFeeModel-" + n_ID;
			...
			return (IPSP.Model.TCode_TRenewalFee)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_TRenewalFee> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_TRenewalFee> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_TRenewalFee> modelList = new List<IPSP.Model.TCode_TRenewalFee>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_TRenewalFee model;
