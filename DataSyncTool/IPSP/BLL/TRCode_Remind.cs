...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TRCode_Remind dal=new IPSP.DAL.TRCode_Remind();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TRCode_Remind GetModelByCache(Guid g_ID)
		{
			
			string CacheKey = "TRCode_RemindModel-" + g_ID;
			...
			return (IPSP.Model.TRCode_Remind)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TRCode_Remind> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TRCode_Remind> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TRCode_Remind> modelList = new List<IPSP.Model.TRCode_Remind>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TRCode_Remind model;
