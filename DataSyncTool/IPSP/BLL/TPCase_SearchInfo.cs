...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TPCase_SearchInfo dal=new IPSP.DAL.TPCase_SearchInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TPCase_SearchInfo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TPCase_SearchInfoModel-" + n_ID;
			...
			return (IPSP.Model.TPCase_SearchInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_SearchInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_SearchInfo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TPCase_SearchInfo> modelList = new List<IPSP.Model.TPCase_SearchInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TPCase_SearchInfo model;
