...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_EmailTemplate dal=new IPSP.DAL.TCode_EmailTemplate();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_EmailTemplate GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_EmailTemplateModel-" + n_ID;
			...
			return (IPSP.Model.TCode_EmailTemplate)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_EmailTemplate> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_EmailTemplate> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_EmailTemplate> modelList = new List<IPSP.Model.TCode_EmailTemplate>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_EmailTemplate model;
