...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_CPCTemplate dal=new IPSP.DAL.TCode_CPCTemplate();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_CPCTemplate GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CPCTemplateModel-" + n_ID;
			...
			return (IPSP.Model.TCode_CPCTemplate)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CPCTemplate> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CPCTemplate> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_CPCTemplate> modelList = new List<IPSP.Model.TCode_CPCTemplate>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_CPCTemplate model;
