...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_SysTasks dal=new IPSP.DAL.TCode_SysTasks();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_SysTasks GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_SysTasksModel-" + n_ID;
			...
			return (IPSP.Model.TCode_SysTasks)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SysTasks> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_SysTasks> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_SysTasks> modelList = new List<IPSP.Model.TCode_SysTasks>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_SysTasks model;
