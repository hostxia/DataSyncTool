...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.T_Demand dal=new IPSP.DAL.T_Demand();
		public T_Demand()
		{}
		#region  BasicMethod
		...
		{
			return dal.Exists(n_ClientID,n_ApplicantID,n_CaseID,n_AgencyID,n_CodeDemandID,n_ID);
		}
		...
		public bool Delete(int n_ClientID,int n_ApplicantID,int n_CaseID,int n_AgencyID,int n_CodeDemandID,int n_ID)
		{
			...
			return dal.Delete(n_ClientID,n_ApplicantID,n_CaseID,n_AgencyID,n_CodeDemandID,n_ID);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.T_Demand GetModelByCache(int n_ID)
		{
			
			string CacheKey = "T_DemandModel-" + n_ID;
			...
			return (IPSP.Model.T_Demand)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_Demand> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.T_Demand> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.T_Demand> modelList = new List<IPSP.Model.T_Demand>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.T_Demand model;
