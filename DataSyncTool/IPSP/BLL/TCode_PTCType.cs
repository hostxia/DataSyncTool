...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_PTCType dal=new IPSP.DAL.TCode_PTCType();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_PTCType GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_PTCTypeModel-" + n_ID;
			...
			return (IPSP.Model.TCode_PTCType)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_PTCType> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_PTCType> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_PTCType> modelList = new List<IPSP.Model.TCode_PTCType>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_PTCType model;
