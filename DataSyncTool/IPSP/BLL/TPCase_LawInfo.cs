...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TPCase_LawInfo dal=new IPSP.DAL.TPCase_LawInfo();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TPCase_LawInfo GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TPCase_LawInfoModel-" + n_ID;
			...
			return (IPSP.Model.TPCase_LawInfo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_LawInfo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_LawInfo> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TPCase_LawInfo> modelList = new List<IPSP.Model.TPCase_LawInfo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TPCase_LawInfo model;
