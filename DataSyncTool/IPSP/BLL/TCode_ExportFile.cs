...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_ExportFile dal=new IPSP.DAL.TCode_ExportFile();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_ExportFile GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_ExportFileModel-" + n_ID;
			...
			return (IPSP.Model.TCode_ExportFile)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_ExportFile> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_ExportFile> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_ExportFile> modelList = new List<IPSP.Model.TCode_ExportFile>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_ExportFile model;
