...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TPCase_AnnFeeStage dal=new IPSP.DAL.TPCase_AnnFeeStage();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TPCase_AnnFeeStage GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TPCase_AnnFeeStageModel-" + n_ID;
			...
			return (IPSP.Model.TPCase_AnnFeeStage)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_AnnFeeStage> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TPCase_AnnFeeStage> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TPCase_AnnFeeStage> modelList = new List<IPSP.Model.TPCase_AnnFeeStage>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TPCase_AnnFeeStage model;
