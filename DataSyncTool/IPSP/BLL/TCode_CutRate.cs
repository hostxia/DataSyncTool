...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_CutRate dal=new IPSP.DAL.TCode_CutRate();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_CutRate GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_CutRateModel-" + n_ID;
			...
			return (IPSP.Model.TCode_CutRate)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CutRate> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_CutRate> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_CutRate> modelList = new List<IPSP.Model.TCode_CutRate>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_CutRate model;
