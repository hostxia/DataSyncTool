...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_Template dal=new IPSP.DAL.TCode_Template();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_Template GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_TemplateModel-" + n_ID;
			...
			return (IPSP.Model.TCode_Template)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Template> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_Template> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_Template> modelList = new List<IPSP.Model.TCode_Template>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_Template model;
