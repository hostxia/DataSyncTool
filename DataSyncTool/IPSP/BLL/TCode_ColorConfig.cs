...
namespace DataSyncTool.IPSP.BLL
{
	...
	{
		private readonly IPSP.DAL.TCode_ColorConfig dal=new IPSP.DAL.TCode_ColorConfig();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public IPSP.Model.TCode_ColorConfig GetModelByCache(int n_ID)
		{
			
			string CacheKey = "TCode_ColorConfigModel-" + n_ID;
			...
			return (IPSP.Model.TCode_ColorConfig)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_ColorConfig> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<IPSP.Model.TCode_ColorConfig> DataTableToList(DataTable dt)
		{
			List<IPSP.Model.TCode_ColorConfig> modelList = new List<IPSP.Model.TCode_ColorConfig>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				IPSP.Model.TCode_ColorConfig model;
