...
namespace DataSyncTool.PC.BLL
{
	...
	{
		private readonly DAL.FILE_TRACKING dal=new DAL.FILE_TRACKING();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.FILE_TRACKING GetModelByCache(string OURNO)
		{
			...
			return (Model.FILE_TRACKING)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.FILE_TRACKING> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.FILE_TRACKING> DataTableToList(DataTable dt)
		{
			List<Model.FILE_TRACKING> modelList = new List<Model.FILE_TRACKING>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.FILE_TRACKING model;
