...
namespace DataSyncTool.PC.BLL
{
	...
	{
		private readonly DAL.FEEDETAIL dal=new DAL.FEEDETAIL();
		public FEEDETAIL()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Model.FEEDETAIL GetModelByCache(string CASENO,string CLIENTNO,string VOUCHERNO,string FEETYPE,decimal AMOUNT,string PAYMETHOD,DateTime PAYDATE)
		{
			...
			return (Model.FEEDETAIL)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.FEEDETAIL> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Model.FEEDETAIL> DataTableToList(DataTable dt)
		{
			List<Model.FEEDETAIL> modelList = new List<Model.FEEDETAIL>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Model.FEEDETAIL model;
