...
namespace DataSyncTool.PC.DAL
{
    ...
    public partial class GENERAL_INSTRUCTION
    {
        ...
        public object GetSingle(string sWhere, string sParaValue)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select PID from GENERAL_INSTRUCTION ");
            if (sWhere.Trim() != "")
            {
                strSql.Append(" where " + sWhere);
            }
            return DbHelperOra.GetSingle(strSql.ToString(), new OracleParameter(":p", OracleType.VarChar) { Value = sParaValue });
        }
