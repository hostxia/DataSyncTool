...
namespace DataSyncTool.BLL
{
	...
	{
		private readonly DataSyncTool.DAL.FEEDETAIL dal=new DataSyncTool.DAL.FEEDETAIL();
		public FEEDETAIL()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public DataSyncTool.Model.FEEDETAIL GetModelByCache(string CASENO,string CLIENTNO,string VOUCHERNO,string FEETYPE,decimal AMOUNT,string PAYMETHOD,DateTime PAYDATE)
		{
			
			string CacheKey = "FEEDETAILModel-" + CASENO+CLIENTNO+VOUCHERNO+FEETYPE+AMOUNT+PAYMETHOD+PAYDATE;
			...
			return (DataSyncTool.Model.FEEDETAIL)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<DataSyncTool.Model.FEEDETAIL> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<DataSyncTool.Model.FEEDETAIL> DataTableToList(DataTable dt)
		{
			List<DataSyncTool.Model.FEEDETAIL> modelList = new List<DataSyncTool.Model.FEEDETAIL>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DataSyncTool.Model.FEEDETAIL model;
