...
namespace DataSyncTool.BLL
{
	...
	{
		private readonly DataSyncTool.DAL.FCASE_WORKLOAD dal=new DataSyncTool.DAL.FCASE_WORKLOAD();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public DataSyncTool.Model.FCASE_WORKLOAD GetModelByCache(string PID)
		{
			...
			return (DataSyncTool.Model.FCASE_WORKLOAD)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<DataSyncTool.Model.FCASE_WORKLOAD> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<DataSyncTool.Model.FCASE_WORKLOAD> DataTableToList(DataTable dt)
		{
			List<DataSyncTool.Model.FCASE_WORKLOAD> modelList = new List<DataSyncTool.Model.FCASE_WORKLOAD>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				DataSyncTool.Model.FCASE_WORKLOAD model;
