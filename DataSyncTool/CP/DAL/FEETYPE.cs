...
namespace DataSyncTool.DAL
{
	...
	public partial class FEETYPE
	{
		...
		public DataSyncTool.Model.FEETYPE GetModel(string TYPENAME)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select TYPENAME,TYPEGROUP from FEETYPE ");
			strSql.Append(" where TYPENAME=:TYPENAME ");
			OracleParameter[] parameters = {
					new OracleParameter(":TYPENAME", OracleType.VarChar,30)			};
			parameters[0].Value = TYPENAME;

			DataSyncTool.Model.FEETYPE model=new DataSyncTool.Model.FEETYPE();
		...
		/// 得到一个对象实体
		/// </summary>
		public DataSyncTool.Model.FEETYPE DataRowToModel(DataRow row)
		{
			DataSyncTool.Model.FEETYPE model=new DataSyncTool.Model.FEETYPE();
			if (row != null)
			{
